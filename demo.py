...
def demo(config):
    ...
    xyz_down, feature = extract_features(model, xyz=np.array(pcd.points), voxel_size=config.voxel_size, device=device,
                                         skip_check=True)

    # convert input to open3D PointCloud data
    vis_pcd = o3d.geometry.PointCloud()
    vis_pcd.points = o3d.utility.Vector3dVector(xyz_down)

    # generate colored features using t-SNE
    ...
    # o3d.visualization.draw_plotly([vis_pcd])

    # save the mesh
    o3d.io.write_triangle_mesh(config.output, vis_pcd)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-o',
        '--output',
        default='processed_cloud.ply',
        type=str,
        help='path to output the processed point cloud')
    parser.add_argument(
        '--voxel_size',
        default=0.025,
        type=float,
